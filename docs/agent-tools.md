...
### 起動前に行を作る（`--session-id`）／長い仕様を渡す（`--prompt-file`）

```console
$ uuid=$(uuidgen | tr 'A-Z' 'a-z')
$ ccs new x01 --session-id "$uuid" --prompt-file ./spec.md
```

- **`--session-id`** — 会話の id を先に決められるので、**起動を待たずに**ハブ側でその行を
  作れる。8-4-4-4-12 の 16 進なら大文字でも受ける（小文字に寄せる）。**既に使われている
  uuid では立てない** ── 同じ会話を 2 本の claude が握ることになる（`ccs adopt` の門と
  同じ理由。design.md §11.2）。そのときは `code: session-id-in-use`
- **`--prompt-file`** — 初期プロンプトをファイルから読む。タスクの仕様は数 KB の複数行に
  なるので、**呼ぶ側の argv に載せない**手立てが要る。`-- <初期プロンプト>` とは併用できない
  （どちらが効いたのかを打った側が確かめられなくなる）。読めなければ `code: prompt-file`
