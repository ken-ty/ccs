...
**ghq 配下は列挙しない。** そこの会話ログは `ccs` が立てたものとは限らない ──
レジストリ全件で `entrypoint` が `cli` / `claude-desktop` / `claude-vscode` に
分かれることは既に確認してある（2026-08-19）。一括で戻すと、`ccs` が管理していない
会話まで tmux に生える。名前で指されたときだけ立てる。
