...
| 決めたこと | 内容 | 理由 |
| --- | --- | --- |
| 置き場所 | `<CCS_WORKTREE_ROOT>/<repo-slug>/<branch-slug>`（既定 `~/.cc-worktrees`） | **ghq root の外に置く。** `ghq list` は `<repo>.worktrees/<name>` のような配置を独立したリポジトリとして列挙する（実測）。ghq 配下に置くと `resolve_as_repo` の末尾一致が worktree にも当たり、`ccs new <branch-slug>` が worktree を掴む。**worktree はリポジトリではない**ので名前解決の名前空間に混ぜない |
| ブランチの slug | `/` を `-` に潰す | tmux のためではなく**パスのため**。潰さないと `feat` と `feat/login` が同じ階層に同居できない |
...
| 入れ子 | worktree の worktree は拒む | どのブランチのどの作業ツリーかが slug から辿れなくなる |
| 後片付け | **`ccs kill` は worktree を消さない** | そこに未コミットの作業が残っている。消す判断は人がする（`ccs gc` での扱いは別項目） |

**根だけは `ccs resolve` でも作る。** macOS の `/tmp` は `/private/tmp` への symlink なので、
実在しないパスは正規化できず、trust に書くキーが噛み合わなくなる（§6 の ghq root で
一度踏んだのと同じ形）。根は空ディレクトリなので作っても害が無い。
**この根は W2 で消える**（[ADR-0003](adr/0003-worktree-under-repo.md) 決定 6）。
...
| 3 | `CCS_WORKTREE_ROOT/<repo>/<branch>` の会話ログ | 同上（C1 で置き場所を `ccs` が決めた。§9.6） |
