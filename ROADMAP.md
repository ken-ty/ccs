...
| worktree | 置き場所は **`<repo>/.worktrees/<branch-slug>`**（`.git/info/exclude` で塞ぐ）。素性は **git から引く**（パス規約を同一性の根拠にしない）。`CCS_WORKTREE_ROOT` は**削除**。**いまの実装は `~/.cc-worktrees` のまま**（W1〜W3） | [ADR-0003](docs/adr/0003-worktree-under-repo.md) |
...
| W2 | **worktree の置き場所を `<repo>/.worktrees/<branch-slug>` へ**（[ADR-0003](docs/adr/0003-worktree-under-repo.md) 決定 1・2・4・5・6）。`.git/info/exclude` に `/.worktrees/` を冪等に書く。worktree を repo 部に指されたら本体へ読み替える。既存ディレクトリの `HEAD` がブランチと食い違えば落とす（`feat-foo` と `feat/foo` の衝突）。**`CCS_WORKTREE_ROOT` を削除**し、`restore_candidates` の候補 3 を `git worktree list --porcelain` へ。テストの差し替え点（`test_helper.bash` / `AGENTS.md`）も落とす。**破壊的変更。移行しない** | M | — （W1 は完了） |
| W3 | **`ccs gc` が worktree を片付ける**（[ADR-0003](docs/adr/0003-worktree-under-repo.md) 決定 7）。4 分類（消す候補 / 未 merge・未 push / dirty / prunable）で報告し、`--yes` で `git worktree remove` → `git branch -d`。**どちらも `--force` / `-D` を使わない** ── 「未 push かつ未マージ」を自動で消さない境界はこの 2 つが持つ。上流の無いブランチは未 push 側に倒す。`ccs kill` は消さず案内 1 行 | S | W2。**I3 も `ccs gc` を触る**ので、先にマージされたほうに合わせる |
