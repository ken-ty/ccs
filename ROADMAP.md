...
| W1 | **worktree の素性判定を git に移す**（[ADR-0003](docs/adr/0003-worktree-under-repo.md) 決定 3）。linked 判定・本体の特定・ブランチ取得を `git rev-parse --absolute-git-dir` / `--git-common-dir` で行い、`resolve_as_worktree` の入れ子チェックと `path_is_ccs_worktree` をパス前方一致から git へ。**`slug_for_path` の穴も塞ぐ** ── いま `ccs resolve <worktree の実パス>` は `mobile-terminal`、`ccs resolve ccs@mobile-terminal` は `ccs@mobile-terminal` を返し、同じ作業ツリーに 2 本立つ（実測）。**置き場所は変えない** | S | — |
| W2 | **worktree の置き場所を `<repo>/.worktrees/<branch-slug>` へ**（[ADR-0003](docs/adr/0003-worktree-under-repo.md) 決定 1・2・4・5・6）。`.git/info/exclude` に `/.worktrees/` を冪等に書く。worktree を repo 部に指されたら本体へ読み替える。既存ディレクトリの `HEAD` がブランチと食い違えば落とす（`feat-foo` と `feat/foo` の衝突）。**`CCS_WORKTREE_ROOT` を削除**し、`restore_candidates` の候補 3 を `git worktree list --porcelain` へ。テストの差し替え点（`test_helper.bash` / `AGENTS.md`）も落とす。**破壊的変更。移行しない** | M | W1 |
