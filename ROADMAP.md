...
| worktree | 置き場所は **`<repo>/.worktrees/<branch-slug>`**（`.git/info/exclude` で塞ぐ）。素性は **git から引く**（パス規約を同一性の根拠にしない）。`CCS_WORKTREE_ROOT` は**削除済み**。**撤去（`ccs gc`）だけ未実装**（W3） | [ADR-0003](docs/adr/0003-worktree-under-repo.md) |
...
| W3 | **`ccs gc` が worktree を片付ける**（[ADR-0003](docs/adr/0003-worktree-under-repo.md) 決定 7）。4 分類（消す候補 / 未 merge・未 push / dirty / prunable）で報告し、`--yes` で `git worktree remove` → `git branch -d`。**どちらも `--force` / `-D` を使わない** ── 「未 push かつ未マージ」を自動で消さない境界はこの 2 つが持つ。上流の無いブランチは未 push 側に倒す。`ccs kill` は消さず案内 1 行 | S | — （W2 は完了）。**I3 も `ccs gc` を触る**ので、先にマージされたほうに合わせる |
