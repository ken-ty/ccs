...
| `CCS_DISMISSED_FILE` | `~/.config/ccs/dismissed` | 一時ファイルに逃がす。**本物を書かせない**（利用者の判断そのもの） |
| `CCS_LAUNCHED_FILE` | `~/.config/ccs/launched` | 一時ファイルに逃がす。**本物を書かせない** |
| `CCS_MCP_LOG_DIR` | OS 依存（macOS は `~/Library/Caches/claude-cli-nodejs`） | 一時ディレクトリに逃がす。**本物を読ませない**（他セッションのログが大量にある） |
